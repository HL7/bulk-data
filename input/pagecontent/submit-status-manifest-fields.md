{% sqlToData submit_status_manifest_fields
	WITH snapshot AS (
		SELECT
		CAST(element.key AS integer) AS ordinal,
		json_extract(element.value, '$.path') AS full_path,
		replace(json_extract(element.value, '$.path'), 'BulkSubmitStatusManifest.', '') AS rel_path,
		...
		WHERE Resources.Id = 'BulkSubmitStatusManifest'
	),
	filtered AS (
		SELECT
		ordinal,
		...
		AND rel_path != 'BulkSubmitStatusManifest'
		AND rel_path != ''
	...
	enriched AS (
		...
		CASE f.seg1
			WHEN 'submissionId' THEN 1
			WHEN 'manifestType' THEN 2
			WHEN 'transactionTime' THEN 3
			WHEN 'epochStartTime' THEN 4
			WHEN 'updateCadence' THEN 5
			WHEN 'requiresAccessToken' THEN 6
			WHEN 'outputFormat' THEN 7
			WHEN 'outputOrganizedBy' THEN 8
			WHEN 'outputOrganizedByDetail' THEN 9
			WHEN 'output' THEN 10
			WHEN 'deleted' THEN 11
			WHEN 'outcome' THEN 12
			WHEN 'link' THEN 13
			ELSE 999
		END AS top_sort,
...
<table class="table">
  ...
  <tbody>
{% for field in submit_status_manifest_fields %}    <tr>
      <td><span style="padding-left: {{ field.indent_px }}px; display: inline-block;">{% if field.level > 0 %}&#8627; {% endif %}<code>{{ field.field_name }}</code></span></td>
